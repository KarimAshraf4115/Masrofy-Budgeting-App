...
export default function SettingsPage() {
  const [isResetting, setIsResetting] = useState(false) // true while deleting
  ...
  const handleReset = async () => {
    ...
    const response = await fetch('/api/cycle', {
      method: 'DELETE'  // calls DELETE /api/cycle
    })

    if (response.ok) {
      setMessage('Cycle reset successfully!')
      setTimeout(() => router.push('/setup'), 1500) // go to setup after 1.5 seconds
    } else {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto w-full space-y-section-gap pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* ========================================== */}
        {/* RIGHT COLUMN: PIN Security (5 cols) */}
        {/* ========================================== */}
        <div className="lg:col-span-5 space-y-8">
          <div className="bg-surface-container-lowest rounded-xl p-card-padding shadow-[0_4px_20px_rgba(0,0,0,0.04)] h-full flex flex-col">
            ...
            <p className="font-body-sm text-body-sm text-on-surface-variant mb-8">Require a PIN to access the application for added security.</p>

            {/* PIN Display Dots */}
            <div className="flex justify-center gap-4 mb-10">
              <div className="w-4 h-4 rounded-full bg-primary-container"></div>
              <div className="w-4 h-4 rounded-full bg-primary-container"></div>
              <div className="w-4 h-4 rounded-full bg-surface-container-highest border border-outline-variant"></div>
              <div className="w-4 h-4 rounded-full bg-surface-container-highest border border-outline-variant"></div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-auto">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                <button key={num} className="h-16 rounded-lg bg-surface hover:bg-surface-container transition-colors font-display-num text-[24px] font-medium text-on-surface flex items-center justify-center">{num}</button>
              ))}
              <div className="h-16 flex items-center justify-center"></div> {/* Empty space */}
              <button className="h-16 rounded-lg bg-surface hover:bg-surface-container transition-colors font-display-num text-[24px] font-medium text-on-surface flex items-center justify-center">0</button>
              <button className="h-16 rounded-lg text-on-surface-variant hover:text-on-surface transition-colors flex items-center justify-center">
